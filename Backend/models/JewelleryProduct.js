...
const productVariantSchema = new mongoose.Schema(
  {
    sku: {
      type: String,
      required: true,
      // ✅ FIXED: removed `unique: true` — a unique index on a nested array
      // field indexes every document (even with an empty array) as null,
      // causing collisions across products with no variants.
      // Uniqueness for variant SKUs should be enforced at the application
      // level (per-product, in addVariant()/updateVariant()), not as a
      // global unique DB index.
    },
    attributes: {
      color: {
        type: String,
...
const jewelleryProductSchema = new mongoose.Schema(
  {
    ...
    // ✅ NEW: Product placements for storefront pages
    placements: {
      type: [String],
      enum: ["shop", "collections", "gifts", "offers"],
      default: [],
      index: true,
    },
...
// Check actual value instead of isModified()
// This works for both create and update operations
// ============================================
jewelleryProductSchema.pre("validate", async function () {
  ...
  // ✅ FIXED: Generate slug if productSlug is empty and productName exists
  // Check actual value instead of using isModified()
  ...
  // If this throws, Mongoose will catch it and surface as validation error
...
jewelleryProductSchema.statics.getProductsBySeller = async function (
  sellerId,
  filters = {},
) {
...
jewelleryProductSchema.statics.getProductLimitStatus = async function (
  sellerId,
) {
  const Seller = mongoose.model("Seller");
...
const JewelleryProduct = mongoose.model(
  "JewelleryProduct",
  jewelleryProductSchema,
);
