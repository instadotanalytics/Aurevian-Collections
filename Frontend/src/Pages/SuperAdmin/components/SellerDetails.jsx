...
// seller: passed directly when navigating from the list (instant render)
// sellerId: route param — used to self-fetch when seller isn't in memory
// (e.g. a hard refresh on /super-admin/dashboard/seller-details/:id)
const SellerDetails = ({ seller: sellerProp, sellerId, onClose }) => {
  const dispatch = useDispatch();
  const [seller, setSeller] = useState(sellerProp || null);
  const [fetchLoading, setFetchLoading] = useState(!sellerProp && !!sellerId);
  ...
  // Keep local state in sync if a fresh seller object is passed in later
  useEffect(() => {
    if (sellerProp) {
  ...
  // Self-fetch when we only have an id (no seller object handed to us yet)
  useEffect(() => {
    if (sellerProp || !sellerId) return;
    ...
    const fetchSeller = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_URL}/super-admin/sellers/${sellerId}`,
  ...
  }, [sellerId, sellerProp, token]);

  const refetchSeller = async () => {
    const id = seller?._id || sellerId;
    if (!id) return;
    try {
      const response = await axios.get(`${API_URL}/super-admin/sellers/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      console.error("Failed to refresh seller:", error);
    }
  };
  ...
  if (fetchError || !seller) {
    return (
      <div className={styles.errorContainer}>
        ...
        <h3>{fetchError || "Seller not found"}</h3>
  ...
  const handleStatusChange = async (status, reasonText = "") => {
    ...
    try {
      ...
      switch (status) {
        case "approved":
          endpoint = `${API_URL}/super-admin/sellers/${seller._id}/approve`;
          break;
        case "rejected":
          endpoint = `${API_URL}/super-admin/sellers/${seller._id}/reject`;
          data = { reason: reasonText || "Application rejected" };
          break;
        case "suspended":
          endpoint = `${API_URL}/super-admin/sellers/${seller._id}/suspend`;
          data = {
          ...
          break;
        case "unsuspend":
          endpoint = `${API_URL}/super-admin/sellers/${seller._id}/unsuspend`;
          break;
        default:
          return;
      }
      ...
      if (response.data.success) {
        ...
        setReason("");
        setSuspendedUntil("");
        // ✅ Refresh just this seller's data instead of reloading the whole page
        await refetchSeller();
      }
    } catch (error) {
      toast.error(
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await axios.put(
        `${API_URL}/super-admin/sellers/${seller._id}/update`,
        editedData,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      if (response.data.success) {
  ...
  // ✅ FIX: model stores this at seller.kyc.status, not seller.verification.kycStatus
  const kycStatus = getKYCStatus(seller.kyc?.status);
    ...
    <>
      <div className={styles.sellerDetails}>
        ...
        <div className={styles.detailsGrid}>
          ...
          <div className={styles.detailsCard}>
            ...
            <div className={styles.detailsContent}>
              <div className={styles.detailRow}>
                ...
                <span className={styles.detailValue}>
                  {seller.businessAddress?.street || "N/A"}
                </span>
              </div>
              <div className={styles.detailRow}>
                <span className={styles.detailLabel}>City</span>
                <span className={styles.detailValue}>
                  {seller.businessAddress?.city || "N/A"}
                </span>
              </div>
              <div className={styles.detailRow}>
                <span className={styles.detailLabel}>State</span>
                <span className={styles.detailValue}>
                  {seller.businessAddress?.state || "N/A"}
                </span>
              </div>
              <div className={styles.detailRow}>
                <span className={styles.detailLabel}>Pincode</span>
                <span className={styles.detailValue}>
                  {seller.businessAddress?.pincode || "N/A"}
                </span>
              </div>
              <div className={styles.detailRow}>
                <span className={styles.detailLabel}>Country</span>
                <span className={styles.detailValue}>
                  {seller.businessAddress?.country || "N/A"}
                </span>
              </div>
            </div>
          </div>
              ...
              <div className={styles.detailsCard}>
                ...
                <div className={styles.detailsContent}>
                  <div className={styles.detailRow}>
                    ...
                    <span className={styles.detailValue}>
                      {seller.bankDetails?.accountHolderName || "N/A"}
                    </span>
                  </div>
                  <div className={styles.detailRow}>
          ...
          {/* Documents */}
          {seller.documents &&
            Object.values(seller.documents).some((doc) => doc) && (
              <div className={styles.detailsCard}>
                <h4 className={styles.detailsTitle}>
                ...
                <div className={styles.detailsContent}>
                  ...
                  <div className={styles.documentLinks}>
                    ...
                    {seller.documents.aadhaarCard && (
                      <a
                        href={seller.documents.aadhaarCard}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
          ...
          <div className={styles.detailsCard}>
            ...
            <div className={styles.timeline}>
              ...
              {seller.suspendedAt && (
                <div className={styles.timelineItem}>
                  <div
                    className={`${styles.timelineDot} ${styles.suspended}`}
                  />
                  <div className={styles.timelineContent}>
